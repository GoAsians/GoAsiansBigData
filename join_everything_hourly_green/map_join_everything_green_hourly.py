	...
	if (len(words)>0):
		if words[0]!="VendorID": #Data line
			if (len(words)==33): #Data from weather
				...
				try:
					key = words[1].replace('\"','')
					year= str(key.split(" ")[0].split("-")[0])
					month = str(key.split(" ")[0].split("-")[1])
					day = str(key.split(" ")[0].split("-")[2])
					hour = str(key.split(" ")[1].split(":")[0])
					if (len(month)==1):
						month = "0"+str(month)
					if (len(day)==1):
						day = "0"+str(day)
					...
					value = words[0]+','+words[2]+','+words[3]+','+words[4]+','+words[5]+','+words[6]+','+words[7]+','+words[8]+','+words[9]+','+words[10]+','+words[11]+','+words[12]+','+words[13]+','+words[14]+','+words[15]+','+words[16]+','+words[17]+','+words[18]+','+words[19]+','+words[20]
